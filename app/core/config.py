...
class Settings(BaseSettings):
    ...
    # 오해 감지 게이트 튜닝 — 값 근거는 scripts/eval_gate.py (누수 없는 79건 기준)
    #
    ...
    # base=0.35가 KR에서는 27건 중 26건을 통과시키고 US에서는 12건 중 6건만 통과시킨다.
    # 전역 상수 하나로는 두 언어를 동시에 맞출 수 없다.
    #
    # 문화별 문턱(그 문화의 neutral 시드가 risk 시드와 닮은 정도를 기준선으로)도 재봤는데
    # F1 0.826으로 오히려 나빴다. 문화당 표본이 1~27건으로 얇아 상수 하나로 대표가 안 된다.
    # 같은 실험을 반복하지 않도록 적어둔다. 근본 해결은 한국어 변별력이 나은 임베딩 모델이다.
    gate_base_threshold: float = 0.35  # 격자 최고점은 0.30 (margin 0.05와 함께 F1 0.887)
