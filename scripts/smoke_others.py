...
async def main() -> None:
    ...
    treq = TranslateRequest(
        utterance_id="u1", source_text="금요일까지 자료 공유 부탁드립니다", source_lang="ko",
    ...
    mreq = MinutesRequest(
        ...
        utterances=[
            {"utterance_id": "u1", "speaker": "종윤", "lang": "ko", "text": "로그인 API는 금요일까지 제가 마무리하겠습니다"},
            {"utterance_id": "u2", "speaker": "Minh", "lang": "vi", "text": "Tôi sẽ lo phần giao diện đăng nhập"},
            {"utterance_id": "u3", "speaker": "수민", "lang": "ko", "text": "디자인 시안은 수요일에 공유할게요"},
            {"utterance_id": "u4", "speaker": "종윤", "lang": "ko", "text": "그럼 다음 회의는 금요일 오후 3시로 하겠습니다"},
        ],
    )
    mr = await generate_minutes(mreq)
    for s in mr.minutes:
        print(f"\n  [{s.language} / {s.job_role}]")
